...
class WC_Gateway_WpsgAch extends WC_Payment_Gateway_eCheck
{
    ...
    function init_form_fields()
    {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => esc_html__('Enable Network Merchants eCheck Payment.', 'woocommerce'),
                'default' => 'no'
            ),
            'title' => array(
                ...
                'default' => esc_html__('eCheck', 'woocommerce')
            ),
            'description' => array(
                ...
                'default' => esc_html__('Pay securely using eCheck', 'woocommerce')
            ),
        );
    }
    ...
    {
        echo '<h3>' . esc_html__('ACH Sandbox Payment Gateway', 'woocommerce') . '</h3>';
    ...
    public function process_payment($order_id)
    {
        ...
        if (
            !isset($_POST[$this->id . '-checkname']) ||
            empty($_POST[$this->id . '-checkname']) ||
            !isset($_POST[$this->id . '-checkaba']) ||
            empty($_POST[$this->id . '-checkaba']) ||
            !isset($_POST[$this->id . '-checkaccount']) ||
            empty($_POST[$this->id . '-checkaccount'])
        ) {
            $this->error_notice(__('All eCheck fields must be filled.', 'woocommerce'));
            return;
        }
